...
def is_code(code: str, lang: str = "python") -> bool:
    ...
    gpt_request = f"The following text should contain python, javascript, bash, shell, or powerscript code. If it does then answer 'yes' otherwise answer 'no'.If you are unsure then your answer is 'yes'./n {code}? "
...
def extract_code(
    ...
    if not detect_single_line_code:
        ...
        code_blocks = [m for m in match if m and is_code(m[1])]  # Single call to is_code
        return code_blocks if code_blocks else [(UNKNOWN, text)]
...
def execute_code(
    ...
    Returns:
        ...
        tmp_file_saved: The message indicating a temp filename was created. Will be empty if filename is provided,
    """
    ...
    if filename is None:
        ...
        tmp_file_saved = f"No filename provided so autogenerated {filename}. "
    if work_dir is None:
            ...
            with ThreadPoolExecutor(max_workers=1) as executor:
                ...
                try:
                    ...
                    if original_filename is None:
                        os.remove(filepath)
                    return 1, TIMEOUT_MSG, None, tmp_file_saved
        logs = limit_output(result.stdout)
        if result.returncode:
            logs += result.stderr
        return result.returncode, logs, filepath, tmp_file_saved
    ...
    if container.status != "exited":
        ...
        if original_filename is None:
            os.remove(filepath)
        return 1, TIMEOUT_MSG, image, tmp_file_saved
    ...
    return exit_code, logs, f"python:{tag}", tmp_file_saved
