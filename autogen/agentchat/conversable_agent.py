...
def register_function(
    ...
    *,
    ...
    # ============================================================================
    # ASYNC NON-RECURSIVE CONVERSATION METHODS (v2)
    # ============================================================================

    async def a_initiate_chat_v2(
        self,
        recipient: "ConversableAgent",
        clear_history: Optional[bool] = True,
        silent: Optional[bool] = False,
        ...
        **context,
        ...
        """(async) Initiate a chat with the recipient agent using non-recursive conversation flow.

        This method provides memory-efficient async conversations by using an event loop
        instead of recursive a_send() -> a_receive() -> a_send() calls. It maintains
        identical functionality to a_initiate_chat() while preventing memory growth
        in long conversations.
        ...
        initial_message = await self.a_generate_init_message(**context)

        # Use AsyncConversationManager for non-recursive async conversation
        async_conversation_manager = AsyncConversationManager(self, recipient)
        await async_conversation_manager.run_conversation(initial_message, silent=silent)
    ...
    async def _a_send_v2(
        ...
        """(async) Send a message to another agent (non-recursive version).

        This method sends a message and processes it on the recipient side
        without triggering the reply chain, allowing the AsyncConversationManager
        to control the conversation flow.
        ...
        Raises:
            ValueError: if the message can't be converted into a valid ChatCompletion message.
        """
        # Same message validation as original a_send()
        valid = self._append_oai_message(message, "assistant", recipient)
        if not valid:
            raise ValueError(
                "Message can't be converted into a valid ChatCompletion message. Either content or function_call must be provided."
    ...
    async def _a_process_and_reply_v2(
        self,
        message: Union[Dict, str],
        sender: Agent,
        request_reply: Optional[bool] = None,
        ...
        """(async) Process received message and generate reply if needed (non-recursive version).

        This method combines message processing and reply generation while
        returning control to the AsyncConversationManager instead of recursively
        calling a_send().
        ...
        Raises:
            ValueError: if the message can't be converted into a valid ChatCompletion message.
        """
        # Check if reply is needed (same logic as original a_receive())
        if request_reply is False or request_reply is None and self.reply_at_receive[sender] is False:
            return False, None

        # Generate reply using existing async generate_reply method
        reply = await self.a_generate_reply(sender=sender)

        # Return whether to continue and the reply
        should_continue = bool(reply)
        return should_continue, reply if reply else None
